...
export async function POST(request: NextRequest) {
  try {
    const { npwpYusen, nomorSeri, npwpPelanggan, nomorKuitansi } =
      await request.json();
    ...
    const normalizedReceiptNumber = nomorKuitansi.trim().toUpperCase();
    ...
    // Compose file name
    const fileNameCompose = `${npwpYusen}-${nomorSeri}-${npwpPelanggan}-${normalizedReceiptNumber}.pdf`;
    console.log("fileName Compose", fileNameCompose);
    ...
    try {
      ...
      // Log for debugging
      // console.log(
      //   `Searching for files with NPWP: ${npwpPelanggan} and Receipt: ${normalizedReceiptNumber}`
      // );
      ...
      const matchingFiles = files.filter((file) => {
        ...
        if (!file.name.includes(fileNameCompose)) {
          return false;
        }
        ...
        const receiptPattern = new RegExp(
          `[^a-z0-9]${normalizedReceiptNumber}[^a-z0-9]|^${normalizedReceiptNumber}[^a-z0-9]|[^a-z0-9]${normalizedReceiptNumber}$|^${normalizedReceiptNumber}$`,
          "i"
        );
        ...
        if (hasExactMatch || hasRegexMatch) {
          ...
          console.log(
            `Match method: ${
              hasExactMatch ? "Exact part match" : "Regex pattern match"
            }`
          );
        }
      ...
      if (matchingFiles.length === 0) {
        ...
        return NextResponse.json(
          {
            error:
              "Faktur tidak ditemukan. Pastikan nomor NPWP dan nomor kuitansi sudah benar dan lengkap.",
          },
          { status: 404 }
        );
      }
